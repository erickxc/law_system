...
async function loadPending() {
    ...
    try {
        ...
        list.innerHTML = `<div class="card"><table class="tbl">
            ...
            <tbody>${users.map(u => `
                <tr id="pending-${u.id}">
                    <td style="color:var(--text); font-weight:500;">${esc(u.full_name)}</td>
                    <td class="mono text-[12px]" style="color:var(--text-3)">${esc(u.email)}</td>
                    <td style="color:var(--text-3)">${esc(u.curso || '—')}</td>
...
function renderBillingList(users) {
    ...
    <button onclick="selectUser('${u.id}','${escAttr(u.full_name)}','${escAttr(u.email)}')" id="billing-user-${u.id}" class="user-list-item">
        <p class="text-[13px] font-medium" style="color:var(--text)">${esc(u.full_name)}</p>
        <p class="mono text-[11px] truncate mt-0.5" style="color:var(--text-4)">${esc(u.email)}</p>
    </button>`).join('');
}
...
function renderPayments(payments) {
    ...
    <tr>
        <td style="color:var(--text); font-weight: 500;">${esc(p.description)}</td>
